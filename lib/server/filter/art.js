...
var sViewNotFound = '<div style="padding: 10px; color: #f00; border: 4px solid #f00; border-radius: 20px;">can not find view: {0}</div>';
...
function renderView(req, dir, depends, path, data) {
	...
	if (!viewPath) {
		return defer.resolve(null, utils.format(sViewNotFound, [path]));
	}
